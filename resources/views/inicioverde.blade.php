...
<body class="bg-gradient-to-br from-green-50 via-white to-emerald-50 dark:from-zinc-900 dark:via-zinc-800 dark:to-zinc-900 text-gray-800 dark:text-neutral-200 antialiased min-h-screen flex flex-col justify-between">
                ...
                @auth
                    <div class="flex items-center gap-3">
                        ...
                        <form method="POST" action="{{ route('logout') }}">
                            @csrf
                            <button type="submit" class="bg-emerald-600 text-white px-4 py-2 rounded-xl text-sm font-semibold hover:bg-emerald-700 transition-all duration-200 shadow-sm cursor-pointer">
                                Cerrar sesión
                            </button>
                        </form>
                    </div>
                @else
                    <a href="{{ route('login') }}" class="text-gray-600 dark:text-neutral-400 hover:text-emerald-700 font-semibold transition-colors">
                        Iniciar Sesión
                    </a>
                    @if (Route::has('register'))
                        <a href="{{ route('register') }}" class="bg-emerald-600 text-white px-5 py-2.5 rounded-xl font-semibold hover:bg-emerald-700 transition-all duration-200 shadow-md">
                            Registrarse
                        </a>
                    @endif
                @endauth
    ...
    <main class="flex-grow flex flex-col items-center justify-center text-center px-6 max-w-4xl mx-auto my-16 md:my-24">
        ...
        <h1 class="text-4xl md:text-6xl font-extrabold text-gray-900 dark:text-neutral-100 tracking-tight leading-none mb-6">
            ...
            <span class="bg-gradient-to-r from-emerald-600 to-green-500 dark:from-violet-500 dark:to-purple-400 bg-clip-text text-transparent">organizados en un solo lugar.</span>
        </h1>
        ...
        <div class="flex flex-col sm:flex-row gap-4 justify-center items-center">
            @auth
                <a href="{{ route('dashboard') }}" class="w-full sm:w-auto bg-emerald-600 text-white px-8 py-4 rounded-xl text-lg font-bold hover:bg-emerald-700 transition-all duration-200 shadow-xl hover:scale-[1.02]">
                    Ir al Panel
                </a>
            @else
                <a href="{{ route('register') }}" class="w-full sm:w-auto bg-emerald-600 text-white px-8 py-4 rounded-xl text-lg font-bold hover:bg-emerald-700 transition-all duration-200 shadow-xl hover:scale-[1.02]">
