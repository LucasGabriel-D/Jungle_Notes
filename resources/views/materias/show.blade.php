<x-layouts::app :title="$materia->nombre">
    <div class="p-6 max-w-5xl mx-auto">
        <div class="mb-6">
            <a href="{{ route('materias.index') }}" class="inline-flex items-center gap-1 text-sm text-neutral-500 dark:text-neutral-400 hover:text-emerald-700 dark:hover:text-violet-400 transition-colors no-underline mb-4">
            ...
            <div class="flex items-start justify-between">
                ...
                <div class="flex items-center gap-2">
                    <a href="{{ route('materias.edit', $materia) }}" class="inline-flex items-center gap-1 text-sm text-neutral-500 dark:text-neutral-400 hover:text-emerald-700 dark:hover:text-violet-400 font-semibold transition-colors no-underline">
                ...
                @forelse($materia->apuntes as $apunte)
                    <div class="px-6 py-4 flex items-center justify-between group">
                        <div class="flex items-center gap-3 min-w-0">
                            <div class="p-2 bg-emerald-50 dark:bg-violet-950/30 text-emerald-700 dark:text-violet-400 rounded-lg shrink-0">
...
<p class="text-xs text-neutral-500 dark:text-neutral-400">por {{ $apunte->user->name }} · {{ $apunte->created_at->diffForHumans() }}</p>
                        ...
                        <a href="{{ asset('storage/' . $apunte->ruta_archivo) }}" target="_blank" class="text-xs bg-emerald-50 dark:bg-violet-950/30 hover:bg-emerald-600 dark:hover:bg-violet-600 text-emerald-700 dark:text-violet-400 hover:text-white px-3 py-2 rounded-lg font-semibold transition-all duration-150 shrink-0">
                            Abrir
                        </a>
                    </div>
                @empty
                    <div class="px-6 py-8 text-center">
                        ...
                        <a href="{{ route('apuntes.index') }}" class="inline-block mt-3 text-sm font-semibold text-emerald-600 dark:text-violet-400 hover:text-emerald-700 dark:hover:text-violet-400 transition-colors">Subir apunte</a>
                    </div>
                @endforelse
            </div>
        </div>
