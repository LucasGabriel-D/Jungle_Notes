<x-layouts::app :title="'Nueva Materia'">
    <div class="p-6 max-w-2xl mx-auto">
        ...
        <div class="bg-white p-6 rounded-xl border border-neutral-200 shadow-sm">
            <h2 class="text-xl font-bold text-neutral-900 mb-6">Nueva Materia</h2>
            ...
            <form method="POST" action="{{ route('materias.store') }}" class="space-y-5">
                ...
                <div>
                    ...
                    <input type="text" name="nombre" value="{{ old('nombre') }}" class="w-full px-4 py-2.5 rounded-xl border border-neutral-200 text-neutral-900 text-sm focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 focus:outline-none transition duration-150" required>
                    @error('nombre') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                </div>

                <div>
                    <label class="block text-sm font-semibold text-neutral-700 mb-1">Descripción</label>
                    <textarea name="descripcion" rows="3" class="w-full px-4 py-2.5 rounded-xl border border-neutral-200 text-neutral-900 text-sm focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 focus:outline-none transition duration-150">{{ old('descripcion') }}</textarea>
